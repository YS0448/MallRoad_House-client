...
const TakeawaySection = ({ activeTab }) => {
  ...
  const fetchTakeawayItems = async (pageNum, search = "") => {
    try {
      setLoading(true);
      let endpoint;
      if (role === "guest") {
        endpoint = `api/takeaway?page=${pageNum}&user_id=null&search=${encodeURIComponent(search)}`;
      } else {
        endpoint = `api/takeaway?page=${pageNum}&user_id=${user.user_id}&search=${encodeURIComponent(search)}`;
      }
      const res = await apiCall("GET", endpoint);
      const items = res.data || [];

      if (items.length < 20) setHasMore(false); // no more data

      setMenuData((prev) => {
        // If page 1 (new search), reset menuData
        if (pageNum === 1) {
          const grouped = {};
          items.forEach((item) => {
        ...
        items.forEach((item) => {
          if (!groupedData[item.category_name]) groupedData[item.category_name] = [];
  ...
  useEffect(() => {
    fetchTakeawayItems(page, searchTerm);
  }, [page, searchTerm]);

  // Reset page to 1 if searchTerm changes
  useEffect(() => {
    setPage(1);
    setHasMore(true);
  }, [searchTerm]);
  ...
  return (
    <div className="takeaway-section container">
      <div className="d-flex justify-content-end">
        <label htmlFor="" className="d-flex align-items-center pe-2 fw-bold">Search</label>
      <input
        type="text"
      ...
      {Object.keys(menuData).map((category, idx, arr) => (
        <div key={category} ref={idx === arr.length - 1 ? lastElementRef : null}>
          <CategorySection category={category} items={menuData[category]} activeTab={activeTab} />
        </div>
      ))}
