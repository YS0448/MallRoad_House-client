...
const DiningSection = ({ activeTab }) => {
  ...
  const fetchDiningItems = async (pageNum, search = "") => {
    try {
      setLoading(true);
      const endpoint = `api/dining?page=${pageNum}&search=${encodeURIComponent(
        search
      )}`;
  ...
  useEffect(() => {
    fetchDiningItems(page, searchTerm);
  }, [page, searchTerm]);
